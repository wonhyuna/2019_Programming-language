...
def is_validated_morse_code(user_input):
    ...
    for k in user_input.split():
        if not k in get_morse_code_dict().values():
            return False
    user_input = user_input.replace("-", "")
    user_input = user_input.replace(".", "")
    user_input = user_input.replace(" ", "")
...
def decoding_character(morse_character):
    ...
    for k, v in morse_code_dict.items():
        for j in morse_character.split():
            if v == j:
                return k
    # ==================================
...
def decoding_sentence(morse_sentence):
    ...
    result = ""
    morse_code_dict = get_morse_code_dict()
    morse_sentence2 = morse_sentence.split(" ")
    for j in morse_sentence2:
        for k, v in morse_code_dict.items():
            if v == j:
                result = result + k
        if j=="":
...
def encoding_sentence(english_sentence):
    ...
    for i in english_sentence:
        if i in "_@#$%^&*()-+=[]{}\"\';:\|`~!.,?" :
            english_sentence=english_sentence.replace(i, "")
